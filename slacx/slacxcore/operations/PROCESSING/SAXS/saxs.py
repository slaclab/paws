...
def perturbation(i0, r0, frac, I, q):
    epsilon = 1e-3
    I_fit = i0 * blur(q * r0, frac) * 9.
    I_i0_plus = i0 * (1 + epsilon) * blur(q * r0, frac) * 9.
    I_i0_minus = i0 * (1 - epsilon) * blur(q * r0, frac) * 9.
    if (chi_squared(I, I_fit) < chi_squared(I, I_i0_plus)) & (chi_squared(I, I_fit) < chi_squared(I, I_i0_minus)):
        print "stable in i0"
    else:
        print "unstable in i0"
    I_r0_plus = i0 * blur(q * r0 * (1 + epsilon), frac) * 9.
    I_r0_minus = i0 * blur(q * r0 * (1 - epsilon), frac) * 9.
    if (chi_squared(I, I_fit) < chi_squared(I, I_r0_plus)) & (chi_squared(I, I_fit) < chi_squared(I, I_r0_minus)):
        print "stable in r0"
    else:
        print "unstable in r0"
    I_frac_plus = i0 * blur(q * r0, frac * (1 + epsilon)) * 9.
    I_frac_minus = i0 * blur(q * r0, frac * (1 - epsilon)) * 9.
    if (chi_squared(I, I_fit) < chi_squared(I, I_frac_plus)) & (chi_squared(I, I_fit) < chi_squared(I, I_frac_minus)):
        print "stable in frac"
    else:
        print "unstable in frac"
        if (chi_squared(I, I_fit) < chi_squared(I, I_frac_plus)):
            print "frac wishes to be lower"
        else:
            print "frac wishes to be higher"
...
np.log(generate_spherical_diffraction)

nameloc = "/Users/Amanda/Data20161118/R1/R1_3rdcool_0004.csv"
...
def testing(nameloc):
    ...
    mean_size = guess_size(fractional_variation, qFirstDip)
    amplitude_at_zero = polydispersity_metric_heightAtZero(qFirstDip, q, I, dI)
    x_vector = q * mean_size
    I_guess_1 = amplitude_at_zero * blur(x_vector, fractional_variation) * 9.
    chi_1 = chi_squared(I, I_guess_1)
    # guess 2
    amplitude_at_zero_g, mean_size_g, fractional_variation_g = refine_guess(q, I, amplitude_at_zero, mean_size,
                                                                      fractional_variation, qFirstDip, heightFirstDip)
    amplitude_at_zero, mean_size, fractional_variation = amplitude_at_zero_g, mean_size_g, fractional_variation_g
    x_vector = q * mean_size
    I_guess_2 = amplitude_at_zero * blur(x_vector, fractional_variation) * 9.
    chi_2 = chi_squared(I, I_guess_2)
    # fit to full data
    popt, pcov = \
        curve_fit(generate_spherical_diffraction, q, I, bounds=([amplitude_at_zero * 0.5, mean_size * 0.5, fractional_variation * 0.1],
                                                                [amplitude_at_zero / 0.5, mean_size / 0.5, fractional_variation / 0.1]))
    amplitude_at_zero_f, mean_size_f, fractional_variation_f = popt[0:3]
    I_fit_full = amplitude_at_zero_f * blur(q * mean_size_f, fractional_variation_f) * 9.
    chi_fit_full = chi_squared(I, I_fit_full)
    # fit to low q only
    lowq = (q < 0.15)
    popt, pcov = \
        curve_fit(generate_spherical_diffraction, q[lowq], I[lowq], bounds=([amplitude_at_zero * 0.5, mean_size * 0.5, fractional_variation * 0.1],
                                                                [amplitude_at_zero / 0.5, mean_size / 0.5, fractional_variation / 0.1]))
    amplitude_at_zero_lo, mean_size_lo, fractional_variation_lo = popt[0:3]
    I_fit_trunc = amplitude_at_zero_lo * blur(q * mean_size_lo, fractional_variation_lo) * 9.
    chi_fit_trunc_low = chi_squared(I[lowq], I_fit_trunc[lowq])
    chi_fit_trunc_all = chi_squared(I, I_fit_trunc)
    # fit to log of data
    popt, pcov = \
        curve_fit(log_diffraction, q, safe_log(I), bounds=([amplitude_at_zero * 0.5, mean_size * 0.5, fractional_variation * 0.1],
                                                                [amplitude_at_zero / 0.5, mean_size / 0.5, fractional_variation / 0.1]))
    amplitude_at_zero_log, mean_size_log, fractional_variation_log = popt[0:3]
    I_fit_log = amplitude_at_zero_log * blur(q * mean_size_log, fractional_variation_log) * 9.
    chi_fit_log = chi_squared(I, I_fit_log)
    ...
    plt.xscale('log')
    plt.yscale('log')
    g1, = plt.plot(q, I_guess_1, 'b-', lw=1)
    g2, = plt.plot(q, I_guess_2, 'g-', lw=1)
    plt.legend((g1, g2),
               ('initial guess, $\chi^2 = $%f' % chi_1, 'refined guess, $\chi^2 = $%f' % chi_2))
    plt.title('Guess and data comparison')
    plt.savefig('/Users/Amanda/Dropbox/Brag_bag/guess_v_data.pdf')
    print "guess results: I0 = %f, r0 = %f, sigma/r0 = %f" %(amplitude_at_zero_g, mean_size_g, fractional_variation_g)
    ...
    plt.xscale('log')
    plt.yscale('log')
    f1, = plt.plot(q, I_fit_full, 'r-', lw=1)
    f2, = plt.plot(q, I_fit_trunc, c='purple')
    f3, = plt.plot(q, I_fit_log, c='orange')
    plt.legend((f1, f2, f3),
               ('fit, $\chi^2 = $%f' % chi_fit_full,'low-q fit, $\chi^2 = $%f' % chi_fit_trunc_all,'log(I) fit, $\chi^2 = $%f' % chi_fit_log))
    plt.title('Fit and data comparison')
    plt.savefig('/Users/Amanda/Dropbox/Brag_bag/guess_v_fit.pdf')
    plt.legend((f1, f2, f3),
               ('fit, $\chi^2 = $%f' % chi_fit_full,
                'low-q fit, $\chi^2 = $%f (low q) or %f (all)' % (chi_fit_trunc_low, chi_fit_trunc_all),
                'log(I) fit, $\chi^2 = $%f' % chi_fit_log))
    print "fit results: I0 = %f, r0 = %f, sigma/r0 = %f" %(amplitude_at_zero_f, mean_size_f, fractional_variation_f)
    perturbation(amplitude_at_zero_f, mean_size_f, fractional_variation_f, I, q)
    return q, I, I_guess_1, I_guess_2, I_fit_full, I_fit_trunc

#q, I, I_guess_1, I_guess_2, I_fit_full, I_fit_trunc = testing(nameloc)
