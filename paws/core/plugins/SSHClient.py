...
class SSHClient(PawsPlugin):
    ...
    def start(self):
        self.run_clone()
        # wait for notification that the clone is running 
    ...
    def request_copy(self,remote_dir,regex,local_dir,block=False):
        ...
        if block:
            if vb: self.message_callback('blocking until files are copied')
            with self.thread_clone.copy_lock:
                self.thread_clone.copy_lock.wait()
            if vb: self.message_callback('finished copy- unblocking')

    def copy_files(self,remote_dir,regex,local_dir,overwrite=False,block=False):
        vb = self.inputs['verbose']
        ...
        for fpath in rdir_list:
            ...
            if fn in ldir_file_list:
                if overwrite:
                    if vb: self.message_callback('overwriting file: {}'.format(fn))
                    self.sftpcl.get(os.path.join(remote_dir,fn),os.path.join(local_dir,fn))
                else:
                    if vb: self.message_callback('{} exists locally: skipping copy'.format(fn))
            else:
