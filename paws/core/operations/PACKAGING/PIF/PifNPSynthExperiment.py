...
class PifNPSynthExperiment(Operation):
    ...
    def run(self):
        ...
        csys.uid = expt_id 
        csys.ids = []
        csys.ids.append(saxs_pif_tools.id_tag('EXPERIMENT_ID',expt_id))
        
        if rcp:
            prep,comp = saxs_pif_tools.process_np_synth_recipe(rcp)
            csys.preparation = prep
            csys.composition = comp

        csys.properties = []
        if t_T is not None:
            csys.properties.append(saxs_pif_tools.time_feature_property('temperature vs. time',t_T,'seconds','degrees C'))
        if t_pars is not None:
            ...
            if t_I0floor.any():
                csys.properties.append(saxs_pif_tools.time_feature_property(
