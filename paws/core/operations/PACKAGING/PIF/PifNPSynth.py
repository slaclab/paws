...
class PifNPSynth(Operation):
    ...
    def saxs_to_pif_properties(self,q_I,T_C):
        ...
        ### property 1: scattering vector
        pq.name = 'SAXS scattering vector'
        pq.scalars = [pifobj.Scalar(q_I[i,0]) for i in range(n_qpoints)]
        pq.conditions = [pifobj.Value('temperature',[pifobj.Scalar(T_C)],None,None,'degrees Celsius')]
        pq.units = '1/Angstrom'
        ### property 2: scattered intensity
        pI = pifobj.Property()
        pI.name = 'SAXS intensity'
        pI.scalars = [pifobj.Scalar(q_I[i,1]) for i in range(n_qpoints)]
        pI.conditions = [pifobj.Value('temperature',[pifobj.Scalar(T_C)],None,None,'degrees Celsius')]
        pI.units = 'counts'
        return [pq,pI] 
