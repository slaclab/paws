...
# TODO: Make this execute chronologically.

# specify workflow names: 
wf_names = ['header_setup','saxs_fit','main']
...
op_maps['main']['batch_headers'] = 'EXECUTION.Batch'
op_maps['main']['sort_header_files'] = 'PACKAGING.BATCH.XYDataFromBatch'
op_maps['main']['batch_fit'] = 'EXECUTION.Batch'
op_maps['main']['fit_params'] = 'PACKAGING.BATCH.XYDataFromBatch'

op_maps['header_setup']['read_header'] = 'IO.BL15.ReadHeader_SSRL15'
op_maps['header_setup']['time_temp'] = 'PACKAGING.BL15.TimeTempFromHeader'
...
op_maps['saxs_fit']['select_populations'] = 'PROCESSING.SAXS.SpectrumClassifierGUI'
...
wf.connect_output('batch_outputs','batch_fit.outputs.batch_outputs')
wf.connect_output('params_vs_t','fit_params.outputs.x_y')

wf.set_op_input('batch_headers','work_item','header_setup','entire workflow')
wf.set_op_input('batch_headers','input_arrays',['header_files.outputs.file_list'],'workflow item')
wf.set_op_input('batch_headers','input_keys',['header_file'])

wf.set_op_input('sort_header_files','batch_outputs','batch_headers.outputs.batch_outputs','workflow item')
wf.set_op_input('sort_header_files','x_key','time')
wf.set_op_input('sort_header_files','y_key','file_path')
wf.set_op_input('sort_header_files','x_sort_flag',True)
wf.set_op_input('sort_header_files','x_shift_flag',True)

wf.set_op_input('batch_fit','work_item','saxs_fit','entire workflow')
wf.set_op_input('batch_fit','input_arrays',['sort_header_files.outputs.y'],'workflow item')
wf.set_op_input('batch_fit','input_keys',['header_file'])

wf.set_op_input('fit_params','batch_outputs','batch_fit.outputs.batch_outputs','workflow item')
...
wf = wfmgr.workflows['header_setup']

# input 0: header file path
...
wf.connect_output('file_path','read_header.inputs.file_path')
...
wf.set_op_input('select_populations','default_pops',{'guinier_porod':1})
wf.connect_input('populations','select_populations.inputs.default_pops')
...
wf.set_op_input('select_populations','q_I','read_spectrum.outputs.q_I','workflow item')
wf.set_op_input('parameterize_spectrum','q_I','read_spectrum.outputs.q_I','workflow item')
wf.set_op_input('parameterize_spectrum','populations','select_populations.outputs.populations','workflow item')
...
pawstools.save_to_wfl(os.path.join(pawstools.sourcedir,'core','workflows','SAXS','BL15','timeseries_flag_and_fit.wfl'),wfmgr)
